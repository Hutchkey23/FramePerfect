...
func show_level_complete_result(result: Dictionary) -> void:
	...
	# Show new best only if medal has been earned previously and player sets high score
	if result.earned_medal and result.medal_already_achieved:
		...
		if "rainbow" in new_best_message:
			play_sfx(NEW_BEST_SFX, NEW_BEST_VOLUME, NEW_BEST_PITCH_RANGE)
		else:
			play_sfx(GOAL_UNLOCK_SFX, GOAL_UNLOCK_VOLUME - 4.0, Vector2(0.6, 0.6))
	
	# If player has not earned a medal previously, show medal earned now or time save needed for medal
	else:
		var medal_message = get_medal_message(result)
		new_best_or_medal_label.text = medal_message
		if "rainbow" in medal_message:
			play_sfx(NEW_BEST_SFX, NEW_BEST_VOLUME, NEW_BEST_PITCH_RANGE)
		else:
			play_sfx(GOAL_UNLOCK_SFX, GOAL_UNLOCK_VOLUME - 4.0, Vector2(0.6, 0.6))
