...
func _ready() -> void:
	...
	level_fail_prompts.position.y += 20
...
func show_level_fail_prompts() -> void:
	...
	tween.tween_property(level_fail_prompts, "position:y", level_fail_prompts_initial_position.y, 0.35).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
