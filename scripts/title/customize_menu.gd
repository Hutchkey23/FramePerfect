...
const LOCKED_SHADER: Shader = preload(
	"res://shaders/locked_cosmetic.gdshader"
)
...
var locked_player_material: ShaderMaterial
var locked_goal_material: ShaderMaterial
...
func _ready() -> void:
	# Materials must exist BEFORE the first preview update.
	setup_locked_materials()
...
func setup_locked_materials() -> void:
	locked_player_material = ShaderMaterial.new()
	locked_player_material.shader = LOCKED_SHADER

	locked_goal_material = ShaderMaterial.new()
	locked_goal_material.shader = LOCKED_SHADER
...
func _on_confirm_button_pressed() -> void:
	...
	# Only unlocked cosmetics can become equipped.
...
func update_player_skin_display(animate: bool = false) -> void:
	...
	var unlocked: bool = SkinDatabase.is_skin_unlocked(skin)

	if unlocked:
		player_skin_label.text = skin.get("display_name", "")
		player_preview.texture = skin["texture"]
		player_preview.material = null
	else:
		player_skin_label.text = LOCKED_NAME

		# Do NOT show the locked skin itself.
		# Use the default player's shape as the silhouette.
		player_preview.texture = SkinDatabase.DEFAULT_PLAYER_TEXTURE
		player_preview.material = locked_player_material
...
func update_goal_skin_display(animate: bool = false) -> void:
	...
	if unlocked:
		goal_skin_label.text = skin.get("display_name", "")
		goal_preview.texture = skin["texture"]
		goal_preview.material = null
	else:
		goal_skin_label.text = LOCKED_NAME

		# Same idea for goals: don't reveal the actual locked skin.
		goal_preview.texture = SkinDatabase.DEFAULT_GOAL_TEXTURE
		goal_preview.material = locked_goal_material
...
func update_message_for_skin(skin: Dictionary) -> void:
	...
	match skin.get("unlock_type", ""):
		...
		"marathon_medal":
			message_label.text = skin.get(
				"locked_message",
				"Earn the marathon medal to unlock!"
			)
