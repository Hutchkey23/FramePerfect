extends Control
class_name MarathonLevelSelect
...
@onready var postcards: Array[MarathonPostcard] = [
...
@onready var all_worlds_background: ColorRect = $BackgroundHolder/AllWorldsBackground
...
func _ready() -> void:
	...
	for i in postcards.size():
		...
		postcards[i].marathon_selected.connect(on_marathon_selected)
...
func on_exit_world(postcard: MarathonPostcard) -> void:
...
func _confirm_world() -> void:
	...
	var selected_card : MarathonPostcard = postcards[current_world_index]
	selected_card.flip_to_back()
	UIAudioManager.play_postcard_flip_to_back_sfx()
	toggle_navigation_arrows(false)

func on_marathon_selected(marathon_data: MarathonData) -> void:
